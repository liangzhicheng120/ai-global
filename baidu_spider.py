...
class BaiduImage():
    def __init__(self, keyword, label, count=2000, save_path="img", rn=60):
        self.keyword = keyword
        self.label = label
        self.count = count
        self.save_path = save_path.decode('utf-8').encode('gbk','ignore')
        self.rn = rn
    ...
    def __save(self, image_url_list, save_path=None):
        ...
        for image in image_url_list:
            ...
            with open(self.save_path + "/{0}_{1}.jpg".format(self.label, self.__totleCount), "wb") as p:
                try:
                    ...
                    p.close()
                    if os.path.exists("img/{0}_{1}.jpg".format(self.label, self.__totleCount)):
                        os.remove("img/{0}_{1}.jpg".format(self.label, self.__totleCount))
...
if __name__ == '__main__':
    ...
    # search = BaiduImage(keyword=word, label=label, save_path='E:\\LearningDeepData\\ai_challenger_scene_other\\{0}\\'.format(word), count=1200)
    # search.search()

    with open('E:\\LearningDeepData\\ai_challenger_scene_other\\scene_classes.txt', 'r') as f:
        for line in f:
            word, label = line.strip().split('\t')
            search = BaiduImage(keyword=word, label=label, save_path='E:\\LearningDeepData\\ai_challenger_scene_other\\{0}\\'.format(word),
                                count=1500)
            search.search()
